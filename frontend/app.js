...
// Count up a number in place, ease-out-cubic. CSS transitions handle bar
// fills automatically via the global prefers-reduced-motion rule in
// styles.css, but a JS-driven text count-up isn't a CSS transition, so
// this checks matchMedia itself and jumps straight to the final value.
function animateScoreNumber(el, target, { duration = 700, decimals = 1, delay = 0 } = {}) {
...
// Trigger any [data-target] width-fill bar inside a container (gauge and
// tile bars alike) -- same double-rAF pattern as animateFormulaFills,
// needed because a transition won't fire if the target state is applied
// in the same frame as the element's insertion into the DOM.
function animateWidthFills(container, selector) {
  requestAnimationFrame(() => {
    container.querySelectorAll(selector).forEach((el) => {
      const target = parseFloat(el.dataset.target) || 0;
      requestAnimationFrame(() => (el.style.width = `${Math.max(target, target > 0 ? 1.5 : 0)}%`));
    });
  });
}

// Fade + translate-up reveal, staggered, for content that should appear
// after the headline score settles (missing sections, suggestions).
function revealStagger(container, selector, { startDelay = 0, stagger = 100 } = {}) {
  const els = container.querySelectorAll(selector);
  els.forEach((el, i) => {
    setTimeout(() => el.classList.add("revealed"), startDelay + i * stagger);
  });
}
...
// ============================================================
// JD Match — now calls /score/full-report directly so the score and the
// full Semantic Fit report render together in one result, from one run.
// ============================================================
document.getElementById("run-jd-match").addEventListener("click", async () => {
  ...
  try {
    ...
    const data = await apiPost("/score/full-report", { resume, jd_text });
    statusEl.textContent = "Done";

    const missingSkillCount = data.keyword_categories.missing_keywords.length;
    resultEl.innerHTML =
      renderFullReport(data) +
      (missingSkillCount
        ? `<div class="action-row" style="margin-top:16px;">
             <button class="link-btn gap-resolution-cta" id="jd-match-resolve-gaps">Resolve gaps conversationally →</button>
           </div>`
        : "");
    animateFormulaFills(resultEl);
    const jdScoreNumberEl = resultEl.querySelector(".score-number");
    if (jdScoreNumberEl) animateScoreNumber(jdScoreNumberEl, data.overall_score, { duration: 700 });
    revealStagger(resultEl, "[data-reveal]", { startDelay: prefersReducedMotion() ? 0 : 550, stagger: 90 });
...
document.getElementById("run-standalone").addEventListener("click", async () => {
  ...
  try {
    ...
    // Same run, same result: also fetch ranked suggestions (No-JD mode --
    // jd_text omitted). Fetched separately from a different endpoint
    // because the readiness score and the suggestion ranking are
    // deliberately independent computations (see suggestion_engine.py),
    // but shown together so there's one page, one answer, not a second
    // click required.
    let suggestionItems = null;
    let suggestionsError = null;
    try {
      const suggData = await apiPost("/score/suggestions", { resume });
      suggestionItems = suggData.suggestions.map((s) => s.message);
    } catch (suggErr) {
      suggestionsError = suggErr.message;
    }

    statusEl.textContent = "Done";

    const tiles = [
      { label: "Structural completeness", value: data.structural_completeness, cls: "match-fill" },
      { label: "Action-verb density", value: data.action_verb_density },
      { label: "Quantified-metric density", value: data.quantified_metric_density },
      { label: "Active voice", value: data.active_voice_score },
    ];

    resultEl.innerHTML = `
      <div class="score-card">
        <div class="score-headline">
          <span class="score-number" data-count-target="${data.score}">0.0</span>
          <span class="score-label">/ 100 readiness</span>
          <span class="role-badge-v2"><span class="pulse-dot"></span>${escapeHtml(data.inferred_role || "unclassified")}</span>
        </div>
        <div class="gauge-track"><div class="gauge-fill" data-target="${data.score}"></div></div>

        <div class="tile-row">
          ${tiles.map((t) => `
            <div class="readiness-tile-v2">
              <div class="tile-value-v2">${t.value.toFixed(0)}%</div>
              <div class="tile-label-v2">${t.label}</div>
              <div class="tile-track"><div class="tile-fill ${t.cls || ""}" data-target="${t.value}"></div></div>
            </div>`).join("")}
        </div>

        <div class="skill-columns reveal-up" data-reveal="missing">
          <div>
        ...
        <div class="report-section reveal-up" data-reveal="suggestions">
          <p class="skill-col-title">Suggestions, ranked by estimated impact</p>
          ${
            suggestionItems
              ? bulletList(suggestionItems, "suggestions", "No suggestions — this resume is in strong shape.")
              : `<p class="tag-empty">Suggestions unavailable: ${escapeHtml(suggestionsError)}</p>`
          }
        </div>
      </div>`;

    // Motion: score counts up over 700ms ease-out-cubic; gauge and tile
    // bars fill in sync via CSS transition; missing-sections/suggestions
    // fade up 150ms after, staggered 120ms apart. Matches the Figma
    // motion spec (file: "Resume Optimizer — Standalone ATS Redesign").
    // Reduced-motion is respected -- see animateScoreNumber() and the
    // global CSS rule collapsing all transition/animation durations.
    const scoreNumberEl = resultEl.querySelector(".score-number");
    animateScoreNumber(scoreNumberEl, data.score, { duration: 700 });
    animateWidthFills(resultEl, ".gauge-fill, .tile-fill");
    revealStagger(resultEl, "[data-reveal]", { startDelay: prefersReducedMotion() ? 0 : 750, stagger: 120 });
  } catch (err) {
    statusEl.textContent = "Failed";
    statusEl.className = "inline-status err";
    resultEl.innerHTML = errorBox(err);
...
// ============================================================
// Full Report rendering -- shared by the JD Match panel above (which
// calls /score/full-report directly and renders inline) and, in the
// no-JD case, adapted by the Standalone panel below.
// ============================================================
const DIMENSION_LABELS = {
  knockout_requirements: { label: "Knockout requirements", short: "knockout" },
  technical_skills: { label: "Technical skills", short: "skills" },
  semantic_fit: { label: "Semantic fit", short: "semantic" },
  experience_match: { label: "Experience", short: "experience" },
  project_relevance: { label: "Projects", short: "projects" },
  education_match: { label: "Education", short: "education" },
  ats_readability: { label: "ATS readability", short: "ats" },
};

function bulletList(items, kind, emptyText) {
  if (!items || items.length === 0) {
    return `<p class="tag-empty">${escapeHtml(emptyText)}</p>`;
  }
  return `<ul class="bullet-list ${kind}">${items.map((s) => `<li>${escapeHtml(s)}</li>`).join("")}</ul>`;
}

function renderFullReport(data) {
  const dims = Object.entries(data.dimensions).map(([key, d]) => ({
    label: DIMENSION_LABELS[key]?.label || key,
    short: DIMENSION_LABELS[key]?.short || key,
    weight: d.weight,
    value: d.score,
  }));
  ...
  const reqRows = data.requirements
    .map((r) => `
      <div class="requirement-row status-${r.status}">
        <span class="requirement-label">${escapeHtml(r.label)}</span>
        <span class="requirement-meta">${r.kind} · ${r.tier} · ${r.status.replace(/_/g, " ")} · ${r.evidence_strength.replace(/_/g, " ")} evidence · ${r.match_type} match · ${r.confidence} confidence</span>
        <span class="requirement-detail">${escapeHtml(r.detail)}${r.evidence_location ? ` — ${escapeHtml(r.evidence_location)}` : ""}</span>
      </div>`)
    .join("");

  return `
    <div class="score-card">
      <div class="knockout-banner ${data.knockout_risk === "HIGH" ? "high" : "low"}">
        Knockout Risk: ${data.knockout_risk}
      </div>
      <div class="score-headline">
        <span class="score-number" data-count-target="${data.overall_score}">0.0</span>
        <span class="score-label">/ 100 overall</span>
        <span class="classification-pill">${escapeHtml(data.classification)}</span>
      </div>
      ${formulaChart(dims)}

      <div class="report-section reveal-up" data-reveal="1">
        <p class="skill-col-title">Keyword coverage</p>
        <div class="skill-columns">
          <div>
            <p class="skill-col-title match-title">Exact matches</p>
            ${tagList(cats.exact_matches, "match", "None.")}
          </div>
          <div>
            <p class="skill-col-title gap-title">Missing keywords</p>
            ${tagList(cats.missing_keywords, "gap", "None missing.")}
          </div>
          <div>
            <p class="skill-col-title">Semantic matches</p>
            ${tagList(cats.semantic_matches, "warn", "None — see note below on the TF-IDF fallback.")}
          </div>
          <div>
            <p class="skill-col-title">Weak (skills-list only)</p>
            ${tagList(cats.weak_keywords, "warn", "None.")}
          </div>
        </div>
      </div>

      <div class="report-section reveal-up" data-reveal="2">
        <p class="skill-col-title">Requirement-by-requirement detail</p>
        <div class="requirement-list">${reqRows}</div>
      </div>

      <div class="report-section reveal-up" data-reveal="3">
        <div>
          <p class="skill-col-title match-title">Strengths</p>
          ${bulletList(data.strengths, "strengths", "Nothing stood out as a strength yet.")}
        </div>
        <div>
          <p class="skill-col-title gap-title">Where you lack</p>
          ${bulletList(data.where_you_lack, "gaps", "No major gaps found.")}
        </div>
      </div>

      ${data.relevance_gaps.length ? `
      <div class="report-section reveal-up" data-reveal="4">
        <p class="skill-col-title">Possibly unrelated to this role</p>
        ${bulletList(data.relevance_gaps, "warnish", "")}
      </div>` : ""}

      ${data.keyword_stuffing_flags.length ? `
      <div class="report-section reveal-up" data-reveal="5">
        <p class="skill-col-title">Keyword density check</p>
        ${bulletList(data.keyword_stuffing_flags, "warnish", "")}
      </div>` : ""}

      <div class="report-section reveal-up" data-reveal="6">
        <p class="skill-col-title">Suggestions, ranked by estimated impact</p>
        ${bulletList(data.suggestions, "suggestions", "No suggestions — this resume is in strong shape against this JD.")}
      </div>

      <div class="report-section reveal-up" data-reveal="7">
        <div>
          <p class="skill-col-title">Top 3 reasons for this score</p>
          ${bulletList(data.top_reasons_for_score, "gaps", "—")}
        </div>
        <div>
          <p class="skill-col-title">Top 3 improvements needed</p>
          ${bulletList(data.top_improvements_needed, "suggestions", "—")}
        </div>
      </div>
    </div>`;
}
