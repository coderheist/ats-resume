...
def _infer_provider_from_model(model: str) -> str:
    """Model naming is consistent per-provider in this codebase (see
    router.py's TASK_MODEL_MAP): "claude-*" for Claude, "gemini-*" for
    Gemini, everything else (Groq's gpt-oss models) falls through to
    "groq". Used by call sites that only have the model string on hand,
    not the Provider enum itself (generate_feedback_summary's callers
    only ever call get_client_for() without an explicit provider, so
    there's nothing else to key off of here)."""
...
def record_llm_usage(model: str, usage: dict, *, provider: str, user_id: str | None = None) -> None:
    """
    Best-effort usage/cost logging (Phase 7 of the architecture plan) --
    called from deep inside the parsing/feedback call chain
    (resume_extraction.py, feedback_prompt.py), which has no FastAPI
    request context or `Depends(get_db)` session to thread through
    without an invasive refactor of functions that were never DB-aware.
    Opens its own short-lived session instead.

    Never raises: a failure here (DB unavailable, unknown model in
    MODEL_RATES) must never break the actual LLM call whose result the
    caller is already relying on -- this is purely an observability
    side effect, not something request handling depends on.

    user_id is usually None here (this call site doesn't know which
    request/user triggered it) -- aggregate cost visibility across all
    calls still works via UsageLog.user_id being nullable; per-user cost
    attribution for this specific call site would need deeper plumbing,
    a known, stated limitation rather than something forced in awkwardly.
    """
    try:
        breakdown = calculate_cost(model, usage)
    except Exception:  # noqa: BLE001 -- e.g. unknown model; never break the caller over this
        return
    ...
    try:
        ...
        try:
            db.add(UsageLog(
                user_id=user_id, action="llm_call", provider=provider,
            ...
            db.close()
    except Exception:  # noqa: BLE001 -- DB unavailable, etc. -- still never break the caller
        pass
