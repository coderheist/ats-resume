...
# Cosine-similarity floor for treating a bullet as a semantic (not exact)
# match to a requirement -- e.g. JD says "machine learning", resume bullet
# says "built predictive classification models using scikit-learn", with
# zero literal word overlap.
#
# Honest limitation, stated rather than silently glossed over: the
# default embedding provider (embeddings.py's TF-IDF fallback) is purely
# lexical -- it scores ANY two phrases with no shared vocabulary at
# exactly 0.0, by construction (verified against real examples: "machine
# learning" vs. a genuinely related classification-model bullet scores
# 0.000, same as an unrelated bullet). That means this code path is
# correct and ready, but functionally inert -- it will essentially never
# fire -- until EMBEDDING_BACKEND=sbert is configured, where dense
# sentence embeddings actually capture this kind of relatedness. 0.35 is
# a reasonable starting point for that backend; it isn't calibrated
# against real SBERT output and should be revisited once that swap
# happens, not treated as a tuned constant today.
_SEMANTIC_MATCH_THRESHOLD = 0.35
...
def _best_semantic_match(requirement_label: str, bullets: list[BulletIssue]) -> tuple[BulletIssue | None, float]:
    """Best-effort semantic search across bullets when no exact phrase
    match exists. See _SEMANTIC_MATCH_THRESHOLD's docstring for what this
    actually does with the default (TF-IDF) provider vs. SBERT."""
    if not bullets:
        return None, 0.0
    provider = get_embedding_provider()
    vectors = provider.embed([requirement_label] + [b.highlight_text for b in bullets])
    req_vector, bullet_vectors = vectors[0], vectors[1:]
    best_bullet, best_score = None, 0.0
    for bullet, vector in zip(bullets, bullet_vectors):
        score = scaled_similarity(req_vector, vector)
        if score > best_score:
            best_bullet, best_score = bullet, score
    return best_bullet, best_score
...
def _match_skill_requirement(requirement: Requirement, resume: JsonResume) -> RequirementMatch:
    ...
    if not found:
        semantic_bullet, similarity = _best_semantic_match(requirement.label, quality.bullet_issues)
        if semantic_bullet is not None and similarity >= _SEMANTIC_MATCH_THRESHOLD:
            # A genuine semantic hit is real evidence, but inherently
            # less certain than an exact phrase match -- capped at
            # MODERATE regardless of whether the bullet has a metric, and
            # reported as partially matched rather than fully matched, so
            # this never silently outranks a demonstrated exact match.
            return RequirementMatch(
                requirement=requirement,
                status=_elevate_if_knockout(MatchStatus.PARTIALLY_MATCHED, requirement.tier),
                evidence_strength=EvidenceStrength.MODERATE,
                match_type=MatchType.SEMANTIC,
                evidence_location=f"Work Experience: {semantic_bullet.job_title}",
                evidence_text=semantic_bullet.highlight_text,
                detail=f"No exact wording match, but a related bullet was found (similarity {similarity:.2f}).",
            )
