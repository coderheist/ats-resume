...
Stated rather than silently skipped: "ATS Readability" here is structural + contact-field completeness only (reusing `readiness.py`'s existing check). True layout analysis — multi-column detection, tables, embedded images, non-standard headings — needs the original file's layout metadata, which isn't captured anywhere past the parsing step in this codebase today. That's a separate, larger addition, not something worth faking a number for.
...
PYTHONPATH=. pytest tests/ -v          # 246 tests, all passing, no external services needed
...
no npm install) covering all ten endpoints above: resume upload/parse,
JD match, standalone readiness, Top-5 suggestions, bias audit, the
voice-editing agent loop, the interactive gap-resolution loop, and pricing.
Note: `/score/suggestions` is not yet wired into a dedicated frontend
panel -- it's callable and tested at the API layer (see
tests/test_scan_routes.py), same backend-first status the gap-resolution
endpoints had before their panel was added.
