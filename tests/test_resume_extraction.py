...
# Deliberately no section headers, no dates, no contact info -- verified
# to score 0.0 confidence (see parse_confidence.py). Used by tests that
# need the heuristic-first confidence gate to actually escalate to the
# LLM fallback, since SAMPLE_RESUME_TEXT above is now well-formatted
# enough to score high confidence on its own (Phase 1's whole point) and
# won't trigger that path.
MESSY_RESUME_TEXT = (
    "Some person who has worked various jobs over the years doing different "
    "tasks and has some skills but no clear structure to describe them in a "
    "normal resume format at all."
)
...
def test_no_provider_configured_falls_back_to_heuristic_with_warning():
    result = parse_resume_text(MESSY_RESUME_TEXT, prefer_llm=True)
    assert result.parse_method == "heuristic"
    assert any("ANTHROPIC_API_KEY" in w and "advanced" in w for w in result.warnings)


def test_well_formatted_resume_never_attempts_llm_even_with_prefer_llm_true():
    """The actual point of Phase 1: a resume the heuristic parser handles
    confidently shouldn't cost an LLM call just because prefer_llm
    defaults to True and no tier was specified -- confidence gates it,
    not "was a provider technically available"."""
    with patch("app.core.llm.client_factory.get_client_for") as mock_get_client:
...
def test_heuristic_extracts_work_entries_single_line_header():
    ...
    # Normalized to canonical YYYY-MM at extraction time (was "Mar 2021"
    # raw before the date-normalization fix) -- see
    # resume_extraction.py's _normalize_date_token docstring for why:
    # experience_match.py's date parser expects this exact form.
    assert work[0].start_date == "2021-03"
    assert work[0].end_date is None  # "Present" -> None
    assert len(work[0].highlights) == 2
    assert work[1].name == "Fieldstone Labs"
    assert work[1].end_date == "2021-02"
...
def test_llm_extraction_falls_back_to_heuristic_on_malformed_output(monkeypatch):
    ...
    with patch("app.core.llm.client_factory.get_client_for", return_value=(fake_client, "claude-sonnet-5")):
        # Explicit tier forces escalation regardless of heuristic confidence
        # -- this test is about the malformed-output fallback specifically,
        # not confidence-gating (which has its own dedicated tests above),
        # and SAMPLE_RESUME_TEXT is now high-confidence on its own (Phase 1's
        # whole point), so it wouldn't reach the LLM node without this.
        result = parse_resume_text(SAMPLE_RESUME_TEXT, prefer_llm=True, tier="advanced")
    assert result.parse_method == "heuristic"
...
def test_no_tier_falls_back_to_active_provider_env_var(monkeypatch):
    ...
    LLM_PROVIDER, same as before tiers existed -- tested here via a
    low-confidence resume specifically so the confidence gate actually
    escalates to the LLM fallback at all (a well-formatted resume
    wouldn't reach this path regardless of which provider is configured,
    per Phase 1)."""
    ...
    fake_client = _FakeExtractionClient(json.dumps(resume_json))
    with patch("app.core.llm.client_factory.get_client_for", return_value=(fake_client, "gemini-3.6-flash")):
        result = parse_resume_text(MESSY_RESUME_TEXT)  # no tier passed
