...
def test_full_report_accepts_optional_format_risk_count(client):
    payload = {
        "resume": _resume_payload(),
        "jd_text": "Software engineer role requiring React and machine learning experience.",
    }
    without_risk = client.post("/score/full-report", json=payload).json()
    with_risk = client.post("/score/full-report", json={**payload, "format_risk_count": 3}).json()

    assert with_risk["dimensions"]["ats_readability"]["score"] < without_risk["dimensions"]["ats_readability"]["score"]
    assert any("formatting risk" in g.lower() for g in with_risk["where_you_lack"])
