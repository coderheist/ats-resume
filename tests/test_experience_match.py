from app.core.scoring.experience_match import (
    RequiredExperience, date_format_issues, experience_match_score, extract_required_years,
...
def test_consistent_yyyy_mm_dates_flag_nothing():
    resume = JsonResume(work=[Work(name="A", position="Engineer", start_date="2020-01", end_date="2022-06")])
    assert date_format_issues(resume) == []


def test_present_as_end_date_is_not_flagged():
    resume = JsonResume(work=[Work(name="A", position="Engineer", start_date="2020-01", end_date="Present")])
    assert date_format_issues(resume) == []


def test_vague_date_phrasing_is_flagged():
    resume = JsonResume(work=[Work(name="A", position="Engineer", start_date="Summer 2022", end_date="2023-01")])
    issues = date_format_issues(resume)
    assert len(issues) == 1
    assert "Engineer" in issues[0]


def test_year_only_is_considered_consistent():
    resume = JsonResume(work=[Work(name="A", position="Engineer", start_date="2020", end_date="2022")])
    assert date_format_issues(resume) == []
